...
public class Player1 : MonoBehaviour
{
    ...
    public float crouchAttackRange = 0.75f;
    ...
    private void Awake()
    {
        ...
        controls.Player1.Move.performed += ctx => direction = ctx.ReadValue<float>();
        controls.Player1.Jump.performed += ctx => Jump();
        controls.Player1.Crouch.performed += ctx => Crouch(true);
        controls.Player1.Crouch.canceled += ctx => Crouch(false);
    ...
    public void Attack()
    {
        if ((crouch == false) && (jump == false) && (Time.time >= nextNormalAttackTime))
        {
            ...
            {
                enemy.GetComponent<Player2>().TakeDamage(normalAttackDamage);
                break;
            }
        ...
        else if ((crouch == true) && (Time.time >= nextCrouchAttackTime))
        {
            ...
            foreach (Collider2D enemy in hitEnemies)
            {
                enemy.GetComponent<Player2>().TakeDamage(crouchAttackDamage);
                break;
            }
    ...
    public void TakeDamage(int damage)
    {
        ...
        Debug.Log("Player 1 took " + damage + " damage.");
